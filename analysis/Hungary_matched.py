...
# hungarian kappa
def hungarian_acc(ys, gts, n):
    cost_matrix = np.zeros([n,n])

    # find max value assignment
    for i in range(n):
        for j in range(n):
            y = ys[i]
            gt = gts[j]
            cost_matrix[i][j] = ((y==gt)*(gt!=7)).sum()/(gt!=7).sum()
    
    max_value = np.max(cost_matrix)
    cost_matrix_ = max_value-cost_matrix
    ans_pos = hungarian_algorithm(cost_matrix_.copy())
    ans, ans_mat = ans_calculation(cost_matrix, ans_pos)
    return ans/n


# hungarian acc
def hungarian_kappa(ys, gts, n):
    cost_matrix = np.zeros([n,n])
    ...
    for i in range(n):
        for j in range(n):
            ...
            y = y[gt!=7]
            gt = gt[gt!=7]
            cost_matrix[i][j] = sklearn.metrics.cohen_kappa_score(y,gt)
